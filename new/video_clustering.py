...
from analysis.embedding_clustering import RetentionTransformer, extract_video_embeddings, find_optimal_clusters
...
def save_results(vids: list[str], labels: np.ndarray, xy: np.ndarray, display: list[dict], strategy: str, metrics: dict, out: Path) -> None:
    ...
    for i, c in enumerate(clusters):
        ...
        # Use modulo arithmetic to avoid IndexError if num clusters > 10
        color = cmap(i % 10)
...
def main() -> None:
    ...
    if "retention" in strategies:
        ...
        from analysis.embedding_clustering import extract_precomputed_embeddings
        ...
        # Now let's train RetentionTransformer to extract sequence-based embeddings and concatenate if requested
        seq_emb_matrix = None
        seq_vids = []
        if args.use_dynamic_embs:
            ...
            if not df.empty and feature_cols:
                ...
                # Brief training to get meaningful embeddings
        ...
        # Merge precomputed and sequence embeddings
        combined_vids = sorted(list(set(ext_vids) | set(seq_vids)))
        combined_embs = []
        final_vids = []

        # Map video ID to its index in matrices
        ...
        if len(combined_embs) < 2:
            ...
            # Use KMeans directly on the extracted, high-dimensional precomputed embeddings
