"""Experimental SARIMAX + multimodal LSTM trainer.

Idea:
1. Fit a SARIMAX trend prior on the mean retention curve of each train cluster.
2. For each video, build a cluster-aware SARIMAX baseline sequence.
3. Train a multimodal LSTM to predict residual corrections over that baseline.

This script is intentionally isolated under ``new/`` and does not affect the
main pipeline.
"""
...
from new.model_sarima_multimodal_lstm import SarimaxMultimodalRetentionLSTM
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Experimental SARIMAX + multimodal LSTM")
    a = p.add_argument
    a("--output-dir-features", default="output")
    a("--snapshot-dir", default="data")
    a("--embeddings-root", default="embeddings")
    a("--output-dir", default="new/experiments/sarimax_multimodal_lstm")
    ...
    a("--sarima-grid", default="1,0,1;2,0,1;1,1,1;2,1,1;2,1,2", help="Semicolon-separated SARIMAX order triples p,d,q")
    return p.parse_args()
...
def _parse_order_grid(spec: str) -> list[tuple[int, int, int]]:
    ...
    for item in spec.split(";"):
        ...
        if not item:
            continue
        p, d, q = (int(x) for x in item.split(","))
        out.append((p, d, q))
    return out
...
def plot_training_curve(train_losses, val_losses, out_path):
    ...
    ax.set(xlabel="epoch", ylabel="composite loss", title="SARIMAX + Multimodal LSTM Training Curve")
    ax.legend()
...
def plot_prediction(vid, y_true, y_pred, y_sarimax, split, metrics, out_path):
    ...
    a1.plot(t, y_sarimax, color=C_PURPLE, label="sarimax prior", alpha=0.85, linewidth=1.0)
    a1.plot(t, y_pred, color=C_ORANGE, label="hybrid prediction", alpha=0.9, linewidth=1.2)
...
class SarimaxMultimodalWindowedDataset(Dataset):
    def __init__(
        self,
        video_dfs: dict[str, pd.DataFrame],
        video_embeddings: dict[str, np.ndarray],
        sarimax_baselines: dict[str, np.ndarray],
        video_ids: list[str],
        ...
        for vid in video_ids:
            ...
            if n <= window_size:
                self.windows.append((emb, X, y, is_ad_col, spike_triggers, base, n, w, 0, ts_opt, n))
            else:
    ...
    def __getitem__(self, idx: int):
        emb, X, y, is_ad, spike_triggers, base, real_len, weight, start, ts_full, n_full = self.windows[idx]
        ws = self.window_size
        if len(X) < ws:
            pad = ws - len(X)
...
def train_model(model, train_dl, val_dl, device, args, use_engagement_weight=True):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_dl:
            emb, tab, tgt, pad_mask, ad_mask, spike_triggers, baseline = _to_device(
                batch, device, "embeddings", "tabular", "retention", "padding_mask", "is_ad", "spike_triggers", "sarimax_baseline"
            )
            vw = batch["video_weight"].to(device) if use_engagement_weight else None
            pred = model(emb, tabular=tab, sarimax_baseline=baseline, src_key_padding_mask=pad_mask)
            loss = composite_loss(
                pred,
                tgt,
                ad_mask,
...
@torch.no_grad()
def predict_video_sarimax_hybrid(
    model: torch.nn.Module,
    df: pd.DataFrame,
    emb: np.ndarray | None,
    feature_cols: list[str],
    normalizer: FeatureNormalizer | None,
    sarimax_baseline_raw: np.ndarray,
    device: torch.device,
    window_size: int = 128,
    stride: int = 1,
    emb_dim: int = 1536,
    ...
    if n <= window_size:
        ...
        pred = model(t_emb, tabular=t_tab, sarimax_baseline=t_base).squeeze(0).cpu().numpy()[:n]
    else:
        ...
        for s in range(0, n - window_size + 1, stride):
            ...
            p = model(t_emb, tabular=t_tab, sarimax_baseline=t_base).squeeze(0).cpu().numpy()
    ...
    return y_true, pred, sarimax_baseline_raw[: len(y_true)]
...
def main():
    ...
    try:
        ...
        raise SystemExit("statsmodels is required for this SARIMAX experiment. Install it first.") from e
    ...
    Path(args.output_dir, "sarimax_trends.json").write_text(json.dumps(trend_provider.describe(), indent=2, ensure_ascii=False), encoding="utf-8")
    ...
    train_ds = SarimaxMultimodalWindowedDataset(
        video_dfs,
        video_embeddings,
        sarimax_baselines,
        train_ids,
    ...
    val_ds = SarimaxMultimodalWindowedDataset(
        video_dfs,
        video_embeddings,
        sarimax_baselines,
        val_ids,
    ...
    model = SarimaxMultimodalRetentionLSTM(
        hidden_size=args.hidden_size, n_layers=args.n_layers, dropout=args.dropout, n_tabular_features=len(feature_cols) + time_feature_extra_dim(args.time_features)
    ).to(device)
    ...
    for vid in video_ids:
        ...
        y_true, y_pred, y_sarimax = predict_video_sarimax_hybrid(
            model,
            video_dfs[vid],
            video_embeddings.get(vid),
            feature_cols,
            normalizer,
            sarimax_baselines[vid],
            device,
            args.window_size,
        ...
        plot_prediction(vid, y_true, y_pred, y_sarimax, split, m, os.path.join(args.output_dir, "videos", vid, "prediction.png"))

    plot_mae_summary(all_metrics, args.output_dir, model_name="SARIMAX+Multimodal-LSTM")
    ...
    torch.save(
        {
            ...
            "config": vars(args),
            "sarimax_trends": trend_provider.describe(),
        },
        os.path.join(args.output_dir, "sarimax_multimodal_lstm.pt"),
    )
    Path(args.output_dir, "metrics.json").write_text(
        json.dumps(
            {
                "model": "SARIMAX+MultimodalLSTM",
                "feature_cols": feature_cols,
                "train_ids": train_ids,
                "val_ids": val_ids,
