...
class RetentionAugmentation:
    ...
    def apply_random_augmentation(features: np.ndarray, augmentation_prob: float = 0.5) -> np.ndarray:
        """
        Applies random augmentations to a sequence of features.
        features: shape (seq_len, num_features) or scalar for targets.
        """
        ...
        # 4. Temporal Scaling (speed up / slow down slightly via interpolation)
        if np.random.rand() < 0.2 and seq_len > 3:
            ...
            # If new_len > seq_len, truncate. If < seq_len, pad with last value.
            if new_len >= seq_len:
    ...
    def _augment_data(self):
        ...
        for i in range(original_len):
            for _ in range(self.num_augmentations):
                ...
                # Keep target unchanged or apply mild augmentation
