...
class RetentionTransformer(nn.Module):
    ...
    def forward(self, src: torch.Tensor, return_embeddings: bool = False):
        ...
        # position encoding needs [seq_len, batch_size, d_model]
        x = x.transpose(0, 1)
...
def extract_precomputed_embeddings(vids: list[str], emb_dir: Path) -> tuple[np.ndarray, list[str]]:
    ...
    # We want a robust representation, so we'll load the embeddings and pool them
    for vid in vids:
        vid_dir = emb_dir / vid
        ...
        for fname in ["audio_embeddings.npy", "bert_embeddings.npy", "visual_embeddings.npy", "videomae_embeddings.npy", "seg_embeddings.npy"]:
            ...
            if path.exists():
                try:
                    arr = np.load(path)
                    # Simple mean pooling over the sequence length to get a single vector per modality
                    if arr.ndim > 1:
                        pooled = arr.mean(axis=0)
                        # Normalize to give equal weight to each modality
        ...
        if emb_parts:
            # Concatenate all available pooled modalities
    ...
    # Since different videos might have different subsets of embeddings missing (though usually they all have the same),
    # let's find the max length and pad just in case
...
def extract_video_embeddings(df, feature_cols, scaler, model, device):
    ...
    with torch.no_grad():
        for video_id in sorted(df["video_id"].unique()):
            ...
            # Pad to 100 if necessary to match the training max_seq_len (or model independent of seq_len)
...
def find_optimal_clusters(embeddings: np.ndarray, min_clusters: int = 2, max_clusters: int = 10, output_dir: Path = None) -> int:
    ...
    # Don't try to find more clusters than we have samples
    ...
    if output_dir is not None:
        ...
        # We will no longer try to write images natively here due to Kaleido/Playwright sandbox issues
        # We will only save the interactive HTML
        fig.write_html(str(output_dir / "optimal_clusters.html"))

        # Draw and save PNG directly using matplotlib
...
class ClusterAwareTransformer:
    ...
    def train_for_cluster(
        ...
        if not cluster_val_videos and val_videos:
            # Fallback if no val videos in this cluster
        ...
        for epoch in range(epochs):
            ...
            for seq, target in train_loader:
                ...
                # target might be a sequence of the same length or a single value
                # Assuming output is (batch, 1) and target is (batch, seq_len) -> mean of target or similar
